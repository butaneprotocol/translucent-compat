  ...
  function serialize(data2) {
    try {
      if (typeof data2 === "bigint") {
        return C.PlutusData.new_integer(C.BigInt.from_str(data2.toString()));
      } else if (typeof data2 === "string") {
        return C.PlutusData.new_bytes(fromHex(data2));
...
class Utils {
  ...
  credentialToAddress(paymentCredential, stakeCredential) {
    if (stakeCredential) {
      return C.BaseAddress.new(
        networkToId(this.translucent.network),
        paymentCredential.type === "Key" ? C.StakeCredential.from_keyhash(
          C.Ed25519KeyHash.from_hex(paymentCredential.hash)
        ) : C.StakeCredential.from_scripthash(
          C.ScriptHash.from_hex(paymentCredential.hash)
        ),
        stakeCredential.type === "Key" ? C.StakeCredential.from_keyhash(
  ...
  validatorToScriptHash(validator) {
    switch (validator.type) {
      case "Native":
        return C.NativeScript.from_bytes(fromHex(validator.script)).hash().to_hex();
      case "PlutusV1":
...
function getAddressDetails(address) {
  ...
  try {
    const parsedAddress = ((address2) => {
      try {
        return C.ByronAddress.from_bytes(fromHex(address2));
      } catch (_e) {
        try {
          return C.ByronAddress.from_base58(address2);
        } catch (_e2) {
          throw new Error("Could not deserialize address.");
        }
      }
...
class Tx {
  ...
  deregisterStake(rewardAddress, redeemer) {
    this.tasks.push((that) => {
      ...
      const credential = addressDetails.stakeCredential.type === "Key" ? C.StakeCredential.from_keyhash(
  ...
  async complete(options) {
    ...
    for (const redeemerBytes of uplcResults) {
      let redeemer = C.Redeemer.from_bytes(redeemerBytes);
...
function addressFromWithNetworkCheck(address, translucent) {
  ...
  return type === "Byron" ? C.ByronAddress.from_base58(address).to_address() : C.Address.from_bech32(address);
}
...
class Message {
  ...
  signWithPrivateKey(privateKey) {
    ...
    const keyHashOriginal = C.PrivateKey.from_bech32(privateKey).to_public().hash().to_hex();
...
class Emulator {
    ...
    accounts.forEach(({ address, assets, outputData }, index) => {
      ...
      this.ledger[GENESIS_HASH + index] = {
        utxo: {
          ...
          address,
          assets,
          datumHash: outputData?.asHash ? C.hash_plutus_data(
  ...
  submitTx(tx) {
    ...
    keyHashes.forEach(
      (keyHash) => edKeyHashes.add(C.Ed25519KeyHash.from_hex(keyHash))
    );
    const nativeHashes = (() => {
      const scriptHashes = [];
      for (let i = 0; i < (witnesses.native_scripts()?.len() || 0); i++) {
        ...
        if (!verifyNativeScript(
          witness,
          Number.isInteger(lowerBound) ? C.BigNum.from_str(lowerBound.toString()) : void 0,
          Number.isInteger(upperBound) ? C.BigNum.from_str(upperBound.toString()) : void 0)) ;
...
class WalletConnector {
  ...
  async rewardAddress() {
    ...
    const rewardAddress = rewardAddressHex ? C.RewardAddress.from_address(
...
function walletFromSeed(seed, options = { addressType: "Base", accountIndex: 0, network: "Mainnet" }) {
  ...
  const address = options.addressType === "Base" ? C.BaseAddress.new(
    networkId,
    C.StakeCredential.from_keyhash(paymentKeyHash),
    C.StakeCredential.from_keyhash(stakeKeyHash)
  ).to_address().to_bech32(void 0) : C.EnterpriseAddress.new(
    networkId,
    C.StakeCredential.from_keyhash(paymentKeyHash)
  ).to_address().to_bech32(void 0);
  const rewardAddress = options.addressType === "Base" ? C.RewardAddress.new(
    networkId,
    C.StakeCredential.from_keyhash(stakeKeyHash)
  ).to_address().to_bech32(void 0) : null;
  return {
...
class SeedWallet {
  constructor(translucent, seed, options) {
    ...
    const stakeKeyHash = stakeKey ? C.PrivateKey.from_bech32(stakeKey).to_public().hash().to_hex() : "";
...
class ExternalWallet {
  ...
  async rewardAddress() {
    const rewardAddr = !this.walletDetails.rewardAddress && this.addressDetails.stakeCredential ? (() => C.RewardAddress.new(
      this.translucent.network === "Mainnet" ? 1 : 0,
...
class Translucent {
  ...
  static async new(provider, network) {
    ...
    if (provider) {
      ...
      SLOT_CONFIG_NETWORK[translucent.network];
      translucent.txBuilderConfig = C.TransactionBuilderConfigBuilder.new().coins_per_utxo_byte(
...
class Maestro {
  ...
  async getUtxosInternal(addressOrCredential, unit) {
    const queryPredicate = (() => {
      ...
      credentialBech32Query += addressOrCredential.type === "Key" ? C.Ed25519KeyHash.from_hex(addressOrCredential.hash).to_bech32(
        "addr_vkh"
