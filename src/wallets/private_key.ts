...
import { SignedMessage, PrivateKey, Address, RewardAddress, Transaction, Translucent, WalletApi, C, UTxO, paymentCredentialOf, utxoToCore, Delegation, Payload, TxHash } from "../mod";
...
export class PrivateKeyWallet implements AbstractWallet {
    ...
    async getUtxos(): Promise<UTxO[]> {
      return await this.translucent.utxosAt(paymentCredentialOf(await this.address()));
    }
    async getUtxosCore(): Promise<C.TransactionUnspentOutputs> {
      const utxos = await this.translucent.utxosAt(paymentCredentialOf(await this.address()));
    ...
    async signTx(tx: C.Transaction): Promise<C.TransactionWitnessSet> {
      const witness = C.make_vkey_witness(C.hash_transaction(tx.body()), this.priv);
      const txWitnessSetBuilder = C.TransactionWitnessSetBuilder.new();
      txWitnessSetBuilder.add_vkey(witness);
      return txWitnessSetBuilder.build();
    }
    // deno-lint-ignore require-await
    async signMessage(address: Address | RewardAddress, payload: Payload): Promise<SignedMessage> {
      const { paymentCredential, address: { hex: hexAddress } } = this.translucent.utils.getAddressDetails(address);
